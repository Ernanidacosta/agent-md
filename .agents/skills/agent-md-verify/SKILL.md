...
4. For operationally relevant changes, update the structured current state in `memory/progress.md`: one valid `Status`, one current `Task` when required, immediate `Next`, explicit `Blockers`, and no more than five recently completed outcomes.
